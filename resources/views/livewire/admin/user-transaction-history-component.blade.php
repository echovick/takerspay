                ...
                <div class="border-b border-gray-100 p-3">
                    <div class="flex justify-between items-start mb-2">
                        <div>
                            ...
                            <div class="font-medium text-sm text-gray-900 break-all">{{ $transaction->transaction_reference }}</div>
                        </div>
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $this->getStatusColor($transaction->status) }}">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <div class="text-right">
                            <div class="font-semibold text-sm {{ $this->getTypeColor($transaction->transaction_type) }}">
                                {{ $transaction->transaction_type === 'credit' ? '+' : '-' }}{{ number_format($transaction->amount, 2) }}
                            </div>
                            <div class="text-xs text-gray-500">{{ $transaction->wallet ? $transaction->wallet->currency : 'N/A' }}</div>
                        </div>
                    </div>
                    @if($transaction->description)
                        <div class="mt-2 text-xs text-gray-600">{{ Str::limit($transaction->transaction_description, 50) }}</div>
                    @endif
                </div>
            @empty
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 cursor-pointer hover:bg-gray-100 w-40" wire:click="sortBy('created_at')">
                            <div class="flex items-center">
                                Date
                        ...
                        <tr class="bg-white border-b hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    ...
                                    @endif
                                    <span class="font-medium {{ $this->getTypeColor($transaction->transaction_type) }} capitalize">
                            ...
                            <td class="px-6 py-4">
                                <div class="font-medium {{ $this->getTypeColor($transaction->transaction_type) }}">
                                    {{ $transaction->transaction_type === 'credit' ? '+' : '-' }}{{ number_format($transaction->amount, 8) }}
                                </div>
                            ...
                            <td class="px-6 py-4">
                                @if($transaction->wallet)
                                    <div class="text-sm text-gray-900 capitalize">{{ $transaction->wallet->type }} Wallet</div>
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-8 text-center">
                                <div class="text-gray-500">
                                    ...
                                    <h3 class="mt-2 text-sm font-medium text-gray-900">No transactions found</h3>
                                    <p class="mt-1 text-sm text-gray-500">This user has no transaction history matching your current filters.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
