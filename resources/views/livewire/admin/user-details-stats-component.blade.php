<div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="text-xl font-bold text-gray-800">
                            {{ number_format($stats['wallets']['crypto']['balance'], 8) }} {{ $stats['wallets']['crypto']['currency'] }}
                        </p>
                    </div>
                    <div>
                    ...
                    <div class="col-span-2">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="text-xl font-bold text-gray-800">{{ number_format($stats['orders']['total']) }}</p>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-500">Success Rate</p>
                                <p class="text-xl font-bold text-green-600">{{ $stats['orders']['completion_rate'] }}%</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="p-6">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-bold text-gray-800">{{ number_format($stats['transactions']['total']) }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-500">Bank Accounts</p>
                        <p class="text-xl font-bold text-gray-800">{{ number_format($stats['support']['bank_accounts']) }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-500">Support Tickets</p>
                        <div class="flex items-center">
                            <p class="text-xl font-bold text-gray-800">{{ number_format($stats['support']['tickets']) }}</p>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <!-- KYC Status -->
        <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-5 border-l-4 {{ $stats['kyc']['verified'] ? 'border-green-500' : 'border-yellow-500' }}">
            <div class="flex items-center">
                <div class="{{ $stats['kyc']['verified'] ? 'bg-green-100' : 'bg-yellow-100' }} p-3 rounded-full">
                    <svg class="h-6 w-6 {{ $stats['kyc']['verified'] ? 'text-green-600' : 'text-yellow-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
