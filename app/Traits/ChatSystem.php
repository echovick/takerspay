...
trait ChatSystem
{
    ...
    private function handleEnterAmountStep(string $userInput)
    {
        ...
        if ($this->order->type == 'buy' && $this->order->asset == 'crypto') {
            $this->addMessage('Bot', "Noted!. at our rate of {$rate}/$ You will send ₦{$nairaEquivalent} to the account details we will provide after you have confirmed the order. Confirm? (yes/cancel)");
        } else if ($this->order->type == 'sell' && $this->order->asset == 'crypto') {
            $this->addMessage('Bot', "Noted!. at our rate of {$rate}/$ You will receive ₦{$nairaEquivalent} to your naira account after sending {$amount} dollars worth of crypto to the wallet address we will provide after you have confirmed the order. Confirm? (yes/cancel)");
        }
    ...
    private function handleSelectGiftCard(string $userInput)
    {
        $this->data['currency'] = strtoupper($userInput);
        ...
        if ($this->order->type == 'buy' && $this->order->asset == 'giftcard') {
            // Ask user for amount
            $this->addMessage('Bot', 'Buying Giftcard currently under development');
        } else {
            $this->addMessage('Bot', 'Buying Giftcard currently under development');
        }
    }
